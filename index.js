...
function temperatureInF(temp, unit) {
  if (unit.toUpperCase() === "F") {
    return temp.toString() + unit.toUpperCase(); //toString converts input temp number to string
  } else {
 ...
 */
function appleIndex(newArr) {
  return newArr.indexOf("apple");
}
appleIndex(["orange", "grape", "apple", "banana", "mango"]);

//A SECOND WAY TO DO THIS WITH FOR LOOP
 ...
 */
function isItAnApple(fruitArr) {
  let newArr = [];
  for (i = 0; i < fruitArr.length; i++) {
    if (fruitArr[i] === "apple") {
      newArr.push(Boolean(true));
    } else {
      newArr.push(Boolean(false));
    }
  }
  return newArr;
}
...
function getLastCarInfo(arr) {
  const lastInv = arr[arr.length - 1];
  return `This is a ${lastInv.car_make} ${lastInv.car_model}`;
}
...
function getCarInfoById(arr, num) {
  //num stands for index position
  return `This is a ${arr[num - 1].car_make} ${arr[num - 1].car_model}`;
}

// console.log(getCarInfoById(inventory, 10));
